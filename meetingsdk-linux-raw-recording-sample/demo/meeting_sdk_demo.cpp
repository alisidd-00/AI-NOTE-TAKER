...
void CheckAndStartRawRecording(bool isVideo, bool isAudio) {
	...
	if (isVideo || isAudio) {
		...
		if (err2 == SDKERR_SUCCESS) {
			...
			SDKError requestErr = m_pRecordController->RequestLocalRecordingPrivilege();
			if (requestErr == SDKERR_SUCCESS) {
                std::cout << "Requested recording permission from the host." << std::endl;
            } else {
                std::cout << "Failed to request recording permission: " << requestErr << std::endl;
            }
...
void exitAfterDelay(int delayInSeconds) {
...
void onIsHost() {
    std::cout << "Is host now... Going to leave meeting." << std::endl;

    // Create a thread to call exitAfterDelay
    std::thread exitThread(exitAfterDelay, 600); // 600 seconds = 10 minutes
    exitThread.detach();
...
void JoinMeeting()
{
	...
	withoutloginParam.userName = "AI NOTE GENIUS";
...
void AuthMeetingSDK()
{
	...
	if (ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS != sdkErrorResult){
		std::cerr << "AuthSDK:error " << sdkErrorResult << ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS << std::endl;
	}
	else{
